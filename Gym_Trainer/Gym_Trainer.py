...
def gemini_response(input,image,prompt):
    if input !="":
        if image:
            response = image_model.generate_content([input,image,prompt],stream=True, safety_settings={'HARASSMENT':'block_none',})
        else:
            response = image_model.generate_content([input,prompt],stream=True, safety_settings={'HARASSMENT':'block_none'})

    else:
        response = image_model.generate_content([image,prompt],stream=True, safety_settings={'HARASSMENT':'block_none'})
    return response
def main():
    load_dotenv()
