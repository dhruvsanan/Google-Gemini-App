...
cutoff = st.number_input("Enter a cutoff percentage (Ideally: 75)",
                         value=None, placeholder="Type a number...", max_value=100, min_value=1)
            ...
            if jt:
                ...
                for uploaded_file in stqdm(uploaded_files):
                    ...
                    response = get_gemini_response(
                        input_promptjt, pdf_content, jd)
                    if response is not None:
                        percentage_match = re.search(
                            r"Percentage Match: (\d+)%", response).group(1)
                        percentage_match_int = int(percentage_match)
                        percentage_matches.append(
                            (percentage_match_int, uploaded_file.name))
                    else:
                        st.error(
